<?php
	...
	$array = array();
	foreach($db->query('SELECT id, name, comment, occupation, UNIX_TIMESTAMP(timestamp) as timestamp FROM comments where shouldShow=1 order by timestamp desc') as $row) {
		$array[] = $row;
	}
